...
class AudioGoalDataset(Dataset):
    def __init__(
        ...
        rgb_dir="/home/Disk/sound-space/rgb_npy/mp3d",
        ...
        self.observation_dir  = 'data/scene_observations_128/mp3d'
    ...
    def _get_rgb_from_npy(self, scene: str, r: int, angle: int):
        rgb = os.path.join(self.rgb_dir, scene, f"{r}_{angle}.npy")
        if not os.path.exists(rgb):
            return None
        rgb = np.load(rgb)  # (1,128,128) float16
        return rgb
    ...
    def __getitem__(self, item):
        if (self.use_cache and self.data[item] is None) or (not self.use_cache):
            ...
            if self.return_depth:
                ...
                rgb_np = self._get_rgb_from_npy(scene, r, angle)   # expected (3,H,W)
                rgb = torch.from_numpy(np.array(rgb_np, copy=False))
                rgb = rgb.float() / 255.0
                mean = torch.tensor([0.485, 0.456, 0.406], dtype=rgb.dtype, device=rgb.device).view(3, 1, 1)
                std  = torch.tensor([0.229, 0.224, 0.225], dtype=rgb.dtype, device=rgb.device).view(3, 1, 1)
                rgb = (rgb - mean) / std


                inputs_outputs = ([spectrogram, depth,rgb], [theta, distance])
            else:
