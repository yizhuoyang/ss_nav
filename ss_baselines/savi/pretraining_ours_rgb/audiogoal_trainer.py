...
from ss_baselines.savi.pretraining_ours_rgb.audiogoal_predictor import AudioGoalPredictor
from ss_baselines.savi.pretraining_ours_rgb.audiogoal_dataset import AudioGoalDataset
        ...
        for epoch in range(num_epoch):
            ...
            for split in splits:
                ...
                for i, data in enumerate(tqdm(dataloaders[split])):
                    ...
                    optimizer.zero_grad()
                    # forward
                    predicts_doa,predicts_dis = model({input_type: x for input_type, x in zip(['spectrogram','depth','rgb'], inputs)})
                ...
                if split == 'val':
                    self.save_checkpoint(f"laset_epoch.pth", checkpoint={"audiogoal_predictor": model.state_dict()})
