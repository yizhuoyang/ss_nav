...
class PPOTrainer(BaseRLTrainer):
    ...
    def _eval_checkpoint(
        ...
        refiner = StreamingSourceMapFusion(
            ...
            out_dir="debug_plan_vis_test/fusion_stream_debug",
            save_every=1,
        )
        # refiner.occ_mapper = self.envs.workers[0]._env.planner.mapper

        if use_visual:
            vis_fuser = StreamingVisualMapFusion(map_size_m=map_size, res=0.1, use_logodds=False,out_dir="debug_plan_vis_test/fusion_visual_debug",save_every=1,)
        ...
        while (
            ...
            observations, rewards, dones, infos = [list(x) for x in zip(*outputs)]
            # if config.TASK_CONFIG.SIMULATOR.CONTINUOUS_VIEW_CHANGE and 'intermediate' in observations[0]:
            #     for observation in observations[0]['intermediate']:
            #         frame = observations_to_image(observation, infos[0])
            #         rgb_frames[0].append(frame)
            #     del observations[0]['intermediate']
            ...
            if len(self.config.VIDEO_OPTION) > 0:
                if 'rgb_frames' in infos[0]:
                    rgb_frames[0].extend(infos[0]['rgb_frames'])  
                if 'audios' in infos[0]:
                    audios[0].extend(infos[0]['audios'])          
                # frame = observations_to_image(observations[0], infos[0])
                # rgb_frames[0].append(infos[0]['rgb_frames'])
                # audios[0].append(infos[0]['audios'])
                # rgb_frames[0].append(frame)
                # audios[0].append(observations[0]['audiogoal'])
