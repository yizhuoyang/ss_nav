...
class Planner:
    ...
    def plan(self, observation: dict, goal, stop, distribution=None) -> torch.Tensor:
        ...
        # np.save(f"/home/Disk/yyz/sound-spaces/test_vis/{x}_{y}.npy",geometric_map)
        # cur = self._map_index_to_graph_nodes([(x, y)])[0]
        # tgt = self._map_index_to_graph_nodes([(goal[0], goal[1])])[0]
        # print("agent_map_xy:", (x,y), "cur_node:", cur, "inG:", cur in self._graph)
        # print("goal_map_xy:", goal, "tgt_node:", tgt, "inG:", tgt in self._graph)
        # print("anchor:", self.mapper._world_x0, self.mapper._world_z0)
    ...
    def goal_to_intermediate_goal(self, goal_xy):
        """
        goal_xy: (x_goal, y_goal) in GLOBAL MAP INDEX (same coordinate as mapper._x,_y)
        return: intermediate_goal (int) in [0, action_map_size^2)
        """
        map_size = self._action_map_size
        c = map_size // 2
        ...
        # 1) global delta (allocentric pixels)
        dx = x_goal - x
        dy = y_goal - y

        # 2) allocentric -> egocentric (in action_map grid units)
        ex, ey = self.mapper.allocentric_to_egocentric(dx, dy, action_map_res=self._action_map_res)

        # 3) round to nearest grid cell
        ex = int(np.round(ex))
        ey = int(np.round(ey))

        # 4) convert to patch indices
        pg_x = c + ex
        pg_y = c + ey

        # 5) clip to patch boundary (重要：goal 太远时必然被裁到边界)
        pg_x = int(np.clip(pg_x, 0, map_size - 1))
        pg_y = int(np.clip(pg_y, 0, map_size - 1))
